/**
 * nemoClawdd MCP Server — HTTP transport entry point
 *
...
function healthCheck(res: ServerResponse): void {
  ...
  res.end(JSON.stringify({ status: "ok", server: "nemoClawdd MCP", version: "0.1.0" }));
}
...
httpServer.listen(PORT, () => {
  console.error(`nemoClawdd MCP Server running (http) on port ${PORT}`);
});
