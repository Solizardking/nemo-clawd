...
export async function cliMigrate(opts: MigrateOptions): Promise<void> {
  ...
  if (!hostState.exists || !hostState.stateDir) {
    ...
    logger.info("Use 'nemo clawd nemoclawd launch ' for a fresh install.");
    return;
  }
  ...
  logger.info("  nemo clawd nemoclawd status     # Verify everything is healthy");
  ...
  if (skipBackup) {
    ...
    logger.info("  nemo clawd nemoclawd eject ");
  }
}
