...
type NemoclawdConfigDocument = Record<string, unknown>;
...
function resolveStateDir(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.join(resolveHostHome(env), ".nemoclawd");
}
...
function resolveConfigPath(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.join(stateDir, "nemoclawd.json");
}

function loadConfigDocument(configPath: string): NemoclawdConfigDocument | null {
  ...
  return parsed as NemoclawdConfigDocument;
}
...
function defaultWorkspacePath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.join(home, ".nemoclawd", "workspace");
}

function collectExternalRoots(
  config: NemoclawdConfigDocument | null,
  stateDir: string,
...
export function detectHostNemoclawd(env: NodeJS.ProcessEnv = process.env): HostNemoclawdState {
...
function resolveConfigSourcePath(manifest: SnapshotManifest, snapshotDir: string): string {
  if (manifest.hasExternalConfig) {
    return path.join(snapshotDir, "config", "nemoclawd.json");
  }
  return path.join(snapshotDir, "nemoclawd", "nemoclawd.json");
}
...
function prepareSandboxState(snapshotDir: string, manifest: SnapshotManifest): string {
  const preparedStateDir = path.join(snapshotDir, "sandbox-bundle", "nemoclawd");
  rmSync(preparedStateDir, { recursive: true, force: true });
  mkdirSync(path.dirname(preparedStateDir), { recursive: true });
  copyDirectory(path.join(snapshotDir, "nemoclawd"), preparedStateDir);
  ...
  writeFileSync(path.join(preparedStateDir, "nemoclawd.json"), JSON.stringify(config, null, 2));
  return preparedStateDir;
}

export function createSnapshotBundle(
  hostState: HostNemoclawdState,
  logger: PluginLogger,
  ...
  try {
    ...
    const snapshotStateDir = path.join(parentDir, "nemoclawd");
    ...
    if (hostState.configPath && hostState.hasExternalConfig) {
      ...
      copyFileSync(hostState.configPath, path.join(configSnapshotDir, "nemoclawd.json"));
    }
...
export function restoreSnapshotToHost(snapshotDir: string, logger: PluginLogger): boolean {
  ...
  const snapshotStateDir = path.join(snapshotDir, "nemoclawd");
  ...
  try {
    ...
    if (manifest.hasExternalConfig && manifest.configPath) {
      const configSnapshotPath = path.join(snapshotDir, "config", "nemoclawd.json");
