...
config_path = os.path.join(home, '.nemoclawd', 'nemoclawd.json')
...
while time.time() < DEADLINE:
    rc, out, err = run('nemoclawd', 'devices', 'list', '--json')
    if rc != 0 or not out:
        time.sleep(1)
        continue
    try:
    ...
    has_browser = any((d.get('clientId') == 'nemoclawd-control-ui') or (d.get('clientMode') == 'webchat') for d in paired if isinstance(d, dict))
    ...
    if pending:
        ...
        for device in pending:
            ...
            if not request_id:
                continue
            arc, aout, aerr = run('nemoclawd', 'devices', 'approve', request_id, '--json')
            if arc == 0:
                APPROVED += 1
...
echo "[gateway] nemoclawd gateway launched (pid $!)"
start_auto_pair
print_dashboard_urls
