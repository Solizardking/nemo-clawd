...
import { loadPerpsRuntimeConfig } from "./config.js";
...
async function getRuntime(): Promise<ClawdPerpsRuntime> {
  if (!runtime) { runtime = new ClawdPerpsRuntime(loadPerpsRuntimeConfig()); await runtime.init(); }
  return runtime;
}
...
type TH = (a: Record<string, unknown>) => Promise<{ ok: boolean; data: unknown; error?: string }>;
const HANDLERS: Record<string, TH> = {
  ...
  perps_market_order: async (a) => { const r = await getRuntime(); return { ok: true, data: await r.executeMarketOrder({ symbol: String(a.symbol), side: String(a.side) as "buy" | "sell", notionalUsd: Number(a.notionalUsd), slippageBps: a.slippageBps ? Number(a.slippageBps) : undefined }) }; },
  perps_limit_order: async (a) => { const r = await getRuntime(); return { ok: true, data: await r.executeLimitOrder({ symbol: String(a.symbol), side: String(a.side) as "buy" | "sell", priceUsd: Number(a.priceUsd), tokens: Number(a.tokens) }) }; },
...
app.get("/health", async (_r, res) => {
  try { const h = await (await getRuntime()).getRuntimeHealth(); res.json({ status: "healthy", mode: h.mode, markets: h.trackedMarkets }); }
  catch (e) { res.status(503).json({ status: "unhealthy", error: e instanceof Error ? e.message : String(e) }); }
});
...
app.post("/agent:query", async (req, res) => {
  try {
    ...
    if (!tool_name) { res.status(400).json({ ok: false, error: "Missing tool_name" }); return; }
    const h = HANDLERS[tool_name];
    if (!h) { res.status(404).json({ ok: false, error: `Unknown tool: ${tool_name}` }); return; }
...
app.post(["/agent/tool", "/"], async (req, res) => {
  ...
  if (n && HANDLERS[n]) { res.json(await HANDLERS[n](args ?? tool_args ?? {})); return; }
